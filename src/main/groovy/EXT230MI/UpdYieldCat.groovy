...
/*
* - Update yield category record to EXTCAT
*/
public class UpdYieldCat extends ExtendM3Transaction {
  ...
  public UpdYieldCat(MIAPI mi, DatabaseAPI database, MICallerAPI miCaller, LoggerAPI logger, ProgramAPI program) {
    this.mi = mi;
  ...
  public void main() {
    ...
    if (!actionEXTCAT.readLock(EXTCAT, updateEXTCAT)){
  ...
  */
  ...
  /*
   * updateEXTCAT - Callback function to update EXTCAT table
   *
   */
  Closure updateEXTCAT = { LockedResult EXTCAT ->
  	...
  	EXTCAT.set("EXCHNO", EXTCAT.get("EXCHNO").toString().toInteger() +1);
