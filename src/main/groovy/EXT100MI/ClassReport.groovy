...
public class ClassReport extends ExtendM3Transaction {
  ...
  public void main() {
  	...
  	// delete report workfile entries forst , then recreate.
...
EXTCLX.delete();
  ...
  Closure<?> releasedItemProcessor = { DBContainer container ->
   warehouse = container.get("MTWHLO")
   lotnumber = container.get("MTBANO")
   itemnumber = container.get("MTITNO")
   orderno = container.get("MTRIDN")
   orderline = container.get("MTRIDL")
   receivingdate = container.get("MTRGDT")
   sampledocket = container.get("MTBREF") 
   
   
    def params01 = ["ITNO":itemnumber.toString(), "BANO": lotnumber.toString() ] // toString is needed to convert from gstring to string
    String atnr = null
  ...
  // getting attribute entries.
   ...
   def writeheader() {
    ...
    int currentDate = LocalDate.now().format(DateTimeFormatter.ofPattern("yyyyMMdd")).toInteger();
  	...
  	EXTCLX.set("EXSEQ1", 0);
  	EXTCLX.set("EXPUNO", orderno);
  	EXTCLX.set("EXPNLI", orderline.toDouble());
  	EXTCLX.set("EXLTYP", 0);
  	EXTCLX.set("EXLOTS", 1);
  	...
  	EXTCLX.set("EXBREF", sampledocket);
  	EXTCLX.set("EXRGDT", receivingdate.toInteger());
...
//  	
   ...
   // this routine writes detailed lines ONLY if ADJxx attribute values <> 'N.'
